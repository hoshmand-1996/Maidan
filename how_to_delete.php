   ...
   <?php
    if(isset($_GET["error"])){
        if($_GET["error"] == "emptyInput"){
            echo '<div class="alert alert-warning" role="alert"> تکایە کۆدەکەت داغڵ بکە! </div>';
        }
        else if($_GET["error"] == "invalid_post_code"){
            echo '<div class="alert alert-danger" role="alert">ببورە کۆدەکەت هەڵەیە!</div>';
        }
    }
  ?>
   ...
   <div class="text_info">
    <h1>سڕینەوەی پۆست</h1>
    <p>کۆدی سڕینەوە لێرەداغڵ بکە</p>
    <form method="post" action="includes/delete_post_by_user.php" class="delete_form">
        <input type="text" placeholder="      کۆدی نهێنی " class="code" name="code" autocomplete="off">

        <input type="submit" value="delete" name="submit">
    </form>

    <p class="help">ئەگەر کۆدی سڕینەوەتان لەبیرکردووە دەتوانن پەیوەندیمان پێوەبکەن لەڕێگای ئەم ژمارەتەلەفونانەوە بۆهاوکاری کردنتان.</p>
    <p class="phone_number">07501814261</p>
  </div>
   </div>
  </section>
  <!-- End post section -->
